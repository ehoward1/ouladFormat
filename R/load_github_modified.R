...
#' based on the stackoverflow post (Hartman, 2016):
...
#'
#' @param github_data_url The GitHub url of the .RData dataset to be loaded.
#' @param env Environment where the data is to be loaded to.
#'
#' @return
#' Loads .Rdata into the local environment
#'
#' @export
...
#' \dontrun{
#' env = environment()
#' load_github_modified("https://github.com/ehoward1/oulad_data/blob/
#' 5bbf34af9922471385371e0d26133d54015f21a2/studentInfo.RData", env)}
load_github_modified = function(github_data_url, env)
{
  url_len <- nchar(github_data_url)
  if (substr(github_data_url, start = (url_len - 8), stop = url_len) !=
      "?raw=true")
